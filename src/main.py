...
class PortState:
    ...
    def __init__(self, port: str):
        ...
        self.is_changing_sim = False  # Flag para indicar que está cambiando SIM en thread
...
class Orchestrator:
    ...
    def __init__(self, config_manager, logger):
        ...
        self.status_log_interval_minutes = self.config.get('status_log_interval_minutes', 1)
    ...
    def _log_round_status(self):
        ...
        while self.status_logging_active:
            try:
                ...
                released_ports = sum(1 for state in self.port_states.values() if state.is_released())
                waiting_ports = sum(1 for state in self.port_states.values() if state.state == 'waiting_registration')
                failed_ports = sum(1 for state in self.port_states.values() if state.state == 'failed')
                ...
                self.logger.info(f"   • Transcurrido: {elapsed:.1f} min / {self.cambio_fila_minutos} min")
                self.logger.info(f"   • Restante: {remaining:.1f} min")
                
                # Mostrar progreso con advertencia si excede 100%
                progress_pct = (elapsed/self.cambio_fila_minutos*100) if self.cambio_fila_minutos > 0 else 0
                if progress_pct > 150:
                    self.logger.warning(f"   • Progreso: {progress_pct:.1f}% ⚠️  EXCEDIDO - POSIBLE THREAD BLOQUEADO")
                else:
                ...
                self.logger.info(f"   • cambio_fila_minutos: {self.cambio_fila_minutos} min")
                self.logger.info(f"   • sim_intento_operativo: {self.sim_intento_operativo} min")
                self.logger.info(f"   • max_workers: {self.max_workers}")
                self.logger.info(f"   • status_log_interval: {self.status_log_interval_minutes} min")
    ...
    def switch_all_modems_parallel(self):
        ...
        for port in ports:
            ...
            self.logger.info(f"  📌 {port}: Fila {fila}, Col {col}, Phone {numero}, CCID {ccid}")
            
            switch_tasks.append((port, pool_com, col, fila, results))
        
        # Ejecutar switches en paralelo con ThreadPoolExecutor
        self.logger.info(f"🚀 Lanzando {len(switch_tasks)} switches en paralelo (max_workers={self.max_workers})...")
        start_time = time.time()
        
        with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            futures = [executor.submit(self._switch_single_modem, *task) for task in switch_tasks]
    ...
    def _switch_single_modem(self, port, pool_com, col, fila, results):
        ...
        try:
            ...
            switch_result = self.hardware.switch_sim(pool_com, col, fila, modem_port=port)
    ...
    def _initialize_all_pools(self):
        ...
        self.logger.info(f"Inicializando {len(pool_coms)} SimBanks: {', '.join(sorted(pool_coms))}")
        ...
        self.logger.info(f"✅ Inicialización completada: {success_count}/{len(pool_coms)} SimBanks OK")
    ...
    def launch_simclient(self):
        ...
        try:
            ...
            self.logger.info(f"  ✅ [{success_time}] SimClient iniciado (PID: {self.simclient_process.pid})")
    ...
    def open_all_serials(self) -> Dict[str, Any]:
        ...
        if empty_ports:
            self.logger.info(f"  🔍 {len(empty_ports)} puertos vacíos (mantenidos por programa): {', '.join(empty_ports)}")
            ...
            for attempt in range(1, max_retries + 1):
                try:
                    ...
                    if port in empty_ports:
                        self.logger.info(f"  🔒 {port}: Serial abierto (puerto vacío - NO para SimClient)")
                    else:
                    ...
                    if attempt < max_retries:
                        self.logger.warning(f"  ⚠️  {port}: Intento {attempt}/{max_retries} falló, reintentando en {retry_delay}s...")
                        time.sleep(retry_delay)
            
            if not success:
                self.logger.error(f"  ❌ {port}: Error después de {max_retries} intentos: {last_error}")
    ...
    def switch_reboot_and_release_parallel(self, serial_objects: Dict[str, Any]):
        ...
        if empty_ports:
            self.logger.info(f"  ⚠️  {len(empty_ports)} puertos sin lista_pos (buscarán SIMs no registradas)")
        ...
        for port in empty_ports:
            ...
            if not unregistered_slots:
                self.logger.warning(f"⚠️  {port}: Sin lista_pos Y sin SIMs no registradas - OMITIENDO")
                continue
            ...
            self.logger.info(f"  🔍 {port}: {len(unregistered_slots)} SIMs no registradas disponibles")
            
            # Agregar tarea especial para slots vacíos
            empty_port_tasks.append((port, pool_com, unregistered_slots, state, results))
        
        # Ejecutar todas las tareas en paralelo con ThreadPoolExecutor
        total_tasks = len(process_tasks) + len(empty_port_tasks)
        self.logger.info(f"🚀 Lanzando {total_tasks} procesos en paralelo ({len(process_tasks)} normales + {len(empty_port_tasks)} vacíos)")
        start_time = time.time()
        
        # Timestamp global de fin de ronda (todos los threads deben terminar antes)
        self.round_start_time = datetime.now()
        round_end_time = self.round_start_time + timedelta(minutes=self.cambio_fila_minutos)
        self.round_end_time = round_end_time
        self.logger.info(f"⏰ Tiempo límite de ronda: {round_end_time.strftime('%H:%M:%S')} ({self.cambio_fila_minutos}min)")
        
        # Iniciar thread de logging periódico
        self.status_logging_active = True
        self.status_thread = threading.Thread(target=self._log_round_status, daemon=True)
        self.status_thread.start()
        self.logger.info(f"📊 Logging de estado activado (cada {self.status_log_interval_minutes} min)")
        ...
        with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            ...
            futures = [executor.submit(self._process_single_port, *task, round_end_time) for task in process_tasks]
            
            # Lanzar tareas para puertos vacíos (con round_end_time)
            futures.extend([executor.submit(self._process_empty_port, *task, round_end_time) for task in empty_port_tasks])
        ...
        if time_until_round_end <= 0:
            ...
            self.logger.info(f"⏰ Tiempo de ronda alcanzado ({self.cambio_fila_minutos}min) - Finalizando ciclo")
        else:
            # Aún hay tiempo, continuar con retry si hay puertos fallidos
            self.logger.info(f"⏳ Tiempo restante en ronda: {time_until_round_end:.1f}s")
            
            # Identificar puertos sin registro
            failed_ports = [port for port, success in results.items() if not success]
            
            if failed_ports:
                self.logger.info(f"⚠️  {len(failed_ports)} puertos sin registro: {', '.join(failed_ports)}")
                self.logger.info(f"🔄 Continuando intentos con siguientes SIMs...")
                # Reintentar con puertos fallidos durante el tiempo restante
                self._retry_failed_ports(failed_ports, serial_objects, time_until_round_end, results, round_end_time)
            else:
                # Todos se registraron, esperar hasta completar la ronda
                self.logger.info(f"✅ Todos los puertos registrados, esperando fin de ronda...")
    ...
    def _process_single_port(self, port: str, pool_com: str, col: str, fila: int,
                            slot: dict, state: PortState, results: dict, round_end_time: datetime):
        """
        ...
        try:
            ...
            self.logger.info(f"[{start_time.strftime('%H:%M:%S.%f')[:-3]}] [{port}] Iniciando proceso...")
            ...
            if not switch_result['success']:
                self.logger.error(f"[{datetime.now().strftime('%H:%M:%S.%f')[:-3]}] [{port}] ❌ Fallo en comando AT+SWIT ({switch_duration:.2f}s)")
            ...
            self.logger.info(f"[{datetime.now().strftime('%H:%M:%S.%f')[:-3]}] [{port}] ✅ AT+SWIT OK ({switch_duration:.2f}s, intento {switch_result['attempts']})")
            
            # Esperar a que el SimBank complete el cambio físico (aumentado a 8s)
            self.logger.info(f"[{datetime.now().strftime('%H:%M:%S.%f')[:-3]}] [{port}] ⏳ Esperando cambio físico (8s)...")
            time.sleep(8)
            ...
            for switch_attempt in range(max_switch_retries):
                if switch_attempt > 0:
                    self.logger.warning(f"[{port}] 🔄 REINTENTO #{switch_attempt+1} de switch completo (cambio físico falló)")
                    ...
                    self.logger.info(f"[{port}] ✅ Reintento de switch OK (intento {switch_result['attempts']})")
                ...
                if not (state.serial_obj and state.serial_obj.is_open):
                    self.logger.error(f"[{datetime.now().strftime('%H:%M:%S.%f')[:-3]}] [{port}] Serial no disponible para reboot")
                    results[port] = False
                    return
                
                reboot_start = datetime.now()
                self.logger.info(f"[{reboot_start.strftime('%H:%M:%S.%f')[:-3]}] [{port}] 🔄 Reiniciando módem (modelo: {model})...")
                success = self.hardware.reboot_modem(state.serial_obj, model)
                reboot_duration = (datetime.now() - reboot_start).total_seconds()
                
                if not success:
                    self.logger.warning(f"[{datetime.now().strftime('%H:%M:%S.%f')[:-3]}] [{port}] ⚠️  Reboot completado con advertencias ({reboot_duration:.2f}s)")
                else:
                    self.logger.info(f"[{datetime.now().strftime('%H:%M:%S.%f')[:-3]}] [{port}] ✅ Reboot OK ({reboot_duration:.2f}s)")
                
                # Esperar estabilización del módem después del reboot
                self.logger.info(f"[{datetime.now().strftime('%H:%M:%S.%f')[:-3]}] [{port}] ⏳ Esperando estabilización (15s)...")
                time.sleep(15)
                ...
                self.logger.info(f"[{ccid_start.strftime('%H:%M:%S.%f')[:-3]}] [{port}] 🔍 Verificando CCID (intento switch {switch_attempt+1}/{max_switch_retries})...")
                ...
                for attempt in range(ccid_check_retries):
                    ...
                    if identity['success'] and identity['ccid']:
                        ...
                        if actual_ccid == expected_ccid:
                            # ✅ CCID correcto
                            self.logger.info(f"[{port}] ✅ CCID verificado correctamente")
                            ccid_verified = True
                            break
                        else:
                            # ❌ CCID incorrecto
                            ...
                            if attempt < ccid_check_retries - 1:
                                self.logger.info(f"[{port}] 🔄 Esperando {attempt+1}/{ccid_check_retries}...")
                                time.sleep(3)
                                continue
                            else:
                                ...
                                self.logger.error(f"[{port}] ❌ CCID no coincide - Switch físico falló")
                                self.logger.error(f"[{port}] 🔍 Detalles del AT+SWIT:")
                                self.logger.error(f"[{port}]    Comando: {switch_result['command']}")
                                self.logger.error(f"[{port}]    Respuesta: {switch_result['response']}")
                                self.logger.error(f"[{port}]    Intentos exitosos: {switch_result['attempts']}")
                                diagnosis_lines = self.ccid_analyzer.format_diagnosis(analysis)
                        ...
                        self.logger.warning(f"[{port}] ⚠️  No se pudo leer CCID (intento {attempt+1})")
            ...
            if not ccid_verified:
                self.logger.error(f"[{datetime.now().strftime('%H:%M:%S.%f')[:-3]}] [{port}] ❌ Verificación CCID falló")
                results[port] = False
                return
            
            ccid_duration = (datetime.now() - ccid_start).total_seconds()
            self.logger.info(f"[{datetime.now().strftime('%H:%M:%S.%f')[:-3]}] [{port}] ✅ CCID verificado ({ccid_duration:.2f}s)")
            
            # PASO 4: MONITOREO CONTINUO (cada 15s durante cambio_fila_minutos)
            monitor_start = datetime.now()
            self.logger.info(f"[{monitor_start.strftime('%H:%M:%S.%f')[:-3]}] [{port}] 🔍 Iniciando monitoreo continuo (máx {self.cambio_fila_minutos}min)")
            ...
            while self.running:
                ...
                if datetime.now() >= round_end_time:
                    self.logger.warning(f"[{port}] ⏰ Límite de ronda alcanzado ({self.cambio_fila_minutos}min) - Terminando thread")
                    results[port] = False
                    return
                ...
                if datetime.now() - monitor_start >= max_duration:
                    self.logger.info(f"[{port}] ⏰ Tiempo máximo alcanzado ({self.cambio_fila_minutos}min) sin registro")
                    results[port] = False
                    return
                ...
                sim_phone = state.current_sim.get('numero', 'N/A') if state.current_sim else 'N/A'
                sim_ccid = state.current_sim.get('ccid', 'N/A') if state.current_sim else 'N/A'
                
                # Verificar si hay error de comunicación
                if creg == 'ERROR':
                    consecutive_errors += 1
                    self.logger.warning(f"[{port}] ⚠️  Error leyendo CREG (consecutivos: {consecutive_errors}/{max_consecutive_errors})")
                    
                    if consecutive_errors >= max_consecutive_errors:
                        self.logger.error(f"[{port}] ❌ Demasiados errores consecutivos ({consecutive_errors}) - Abandonando puerto")
                        results[port] = False
                        return
                ...
                if creg in ['1', '5']:
                    ...
                    self.logger.info(f"[{port}] ✅ Registrado ({network_type}) en {elapsed_time:.1f}min")
                ...
                if check_count == 1 or check_count % 4 == 0:  # Log cada minuto aprox
                    if creg in ['0', '2', '3']:
                        self.logger.warning(f"[{port}] ⚠️  Ciclo {check_count}: CREG={creg} (sin registro), Phone={sim_phone}, CCID={sim_ccid}, Tiempo={elapsed_time:.1f}min")
                    else:
                        self.logger.warning(f"[{port}] ⚠️  Ciclo {check_count}: CREG={creg}, Phone={sim_phone}, CCID={sim_ccid}, Tiempo={elapsed_time:.1f}min")
                
                # Reboot global a sim_intento_operativo/3
                if elapsed_time >= reboot_threshold and not reboot_done:
                    self.logger.info(f"[{port}] 🔄 Reboot adicional ({reboot_threshold:.1f}min)")
                    self.hardware.reboot_modem(state.serial_obj, model)
                    self.logger.info(f"[{port}] ⏳ Esperando estabilización del módem (15s)...")
                    time.sleep(15)
                    reboot_done = True
                
                # Cambio de SIM a sim_intento_operativo
                if elapsed_time >= change_threshold and not sim_changed:
                    self.logger.warning(f"[{port}] ⏰ {elapsed_time:.1f}min sin registro, cambiando SIM...")
            ...
            self.logger.error(f"[{datetime.now().strftime('%H:%M:%S.%f')[:-3]}] [{port}] Error en thread: {e} (duración total: {total_duration:.1f}s)")
            results[port] = False
        finally:
            ...
            if results.get(port) == True:
                if state.serial_obj and state.serial_obj.is_open:
                    try:
                        ...
                        self.logger.debug(f"[{datetime.now().strftime('%H:%M:%S.%f')[:-3]}] [{port}] Serial cerrado tras registro exitoso (duración: {total_duration:.1f}s)")
                    except Exception as e:
                        self.logger.warning(f"[{datetime.now().strftime('%H:%M:%S.%f')[:-3]}] [{port}] Error al cerrar serial: {e}")
            else:
                # Puerto sin registro - mantener serial abierto para reintentos
                self.logger.debug(f"[{datetime.now().strftime('%H:%M:%S.%f')[:-3]}] [{port}] Serial mantenido abierto para reintentos (duración: {total_duration:.1f}s)")
    
    def _process_empty_port(self, port: str, pool_com: str, unregistered_slots: List[Dict],
                            state: PortState, results: dict, round_end_time: datetime):
        """
        ...
        try:
            start_time = datetime.now()
            self.logger.info(f"[{start_time.strftime('%H:%M:%S.%f')[:-3]}] [{port}] 🔍 Iniciando búsqueda activa ({len(unregistered_slots)} SIMs)...")
            
            # Inicializar índice de rotación
            rotation_index = 0
            sim_attempt_time = self.sim_intento_operativo * 60  # 3 minutos por SIM
            reboot_threshold_time = sim_attempt_time / 3.0  # 1 minuto para reboot adicional
            ...
            while self.running:
                ...
                current_slot = unregistered_slots[rotation_index % len(unregistered_slots)]
                ...
                if time_since_change >= sim_attempt_time:
                    self.logger.info(f"[{port}] ⏱️  {sim_attempt_time/60:.0f} min sin registro - cambiando a siguiente SIM")
                    rotation_index += 1
                    last_change_time = time.time()
                    reboot_done = False
                    continue
                ...
                if time_since_change >= reboot_threshold_time and not reboot_done:
                    self.logger.info(f"[{port}] 🔄 Reboot adicional tras {reboot_threshold_time/60:.0f} min")
                    try:
                        if state.serial_obj and state.serial_obj.is_open:
                        ...
                        self.logger.warning(f"[{port}] ⚠️  Error en reboot adicional: {e}")
                
                # Primera verificación o después de cambio - hacer switch
                if time_since_change < 1:
                    self.logger.info(f"[{port}] 🔄 Probando Fila {fila}, Col {col}, Phone {numero}")
                    
                    # PASO 1: Switch
                    try:
                        switch_result = self.hardware.switch_sim(pool_com, col, fila, modem_port=port)
                        if not switch_result['success']:
                            self.logger.warning(f"[{port}] ⚠️  Switch falló, reintentando...")
                            time.sleep(5)
                            continue
                    ...
                    self.logger.info(f"[{port}] ⏳ Esperando estabilización + registro ({total_wait}s)...")
                    time.sleep(total_wait)
                
                # VERIFICACIÓN ADICIONAL antes de chequear CREG
                if datetime.now() >= round_end_time:
                    self.logger.warning(f"[{port}] ⏰ Límite de ronda alcanzado durante monitoreo - Terminando")
                    results[port] = False
                    return
                ...
                try:
                    ...
                    self.logger.debug(f"[{port}] 📶 CREG={creg} (tiempo: {time_since_change:.0f}s/{sim_attempt_time:.0f}s)")
                    
                    if creg in ['1', '5']:
                        elapsed = (datetime.now() - start_time).total_seconds()
                        self.logger.info(f"[{port}] ✅ ¡PRIMER REGISTRO ENCONTRADO! CREG={creg}, Fila {fila}, Phone {numero}")
                        self.logger.info(f"[{port}] 🗺️  Iniciando mapeo completo de todas las SIMs no registradas...")
                        ...
                        if registered_slots:
                            self.logger.info(f"[{port}] 🎓 Mapeo completo: {len(registered_slots)}/{len(unregistered_slots)} SIMs se registraron")
                            
                            # GRADUAR a producción
                            self.slot_manager.add_port_to_production(port, registered_slots)
                            
                            # Ahora SÍ liberar a SimClient
                            self.logger.info(f"[{port}] 🚀 Puerto graduado, liberando a SimClient...")
                            results[port] = True  # True = liberar
                            
                            # Cerrar serial para que SimClient lo tome
                            if state.serial_obj and state.serial_obj.is_open:
                                state.serial_obj.close()
                                self.logger.info(f"[{port}] ✅ Serial cerrado, disponible para SimClient")
                        else:
                            self.logger.warning(f"[{port}] ⚠️  Mapeo completo falló, no se encontraron SIMs registradas")
                            results[port] = False
                    ...
                    if datetime.now() >= round_end_time:
                        self.logger.warning(f"[{port}] ⏰ Límite de ronda alcanzado - Terminando empty_port")
                        results[port] = False
                        return
            ...
            self.logger.debug(f"[{port}] 🔒 Thread empty_port terminado, serial mantenido abierto")
    
    def _complete_mapping_for_empty_port(self, port: str, pool_com: str, 
                                         unregistered_slots: List[Dict],
                                         state: PortState) -> List[Dict]:
        """
        ...
        for idx, slot in enumerate(unregistered_slots, 1):
            ...
            self.logger.info(f"[{port}] [{idx}/{total}] Probando Fila {fila}, Col {col}, Phone {numero}")
            ...
            try:
                ...
                self.logger.info(f"[{port}] [{idx}/{total}] ⏳ Esperando {total_wait}s...")
                time.sleep(total_wait)
                ...
                if creg in ['1', '5']:
                    self.logger.info(f"[{port}] [{idx}/{total}] ✅ REGISTRADA - CREG={creg}")
                    registered_slots.append(slot)
                else:
                    self.logger.info(f"[{port}] [{idx}/{total}] ❌ No registrada - CREG={creg}")
            
            except Exception as e:
                self.logger.warning(f"[{port}] [{idx}/{total}] ⚠️  Error: {e}")
        
        self.logger.info(f"[{port}] 🗺️  Mapeo completado: {len(registered_slots)}/{total} registradas")
        return registered_slots
    
    def _retry_failed_ports(self, failed_ports: List[str], serial_objects: Dict, 
                           remaining_time: float, results: dict, round_end_time: datetime):
        """
        ...
        if remaining_time <= 0:
            self.logger.warning(f"⚠️  Sin tiempo restante para reintentos (remaining={remaining_time:.1f}s)")
            return
        
        # Establecer timeout mínimo de 30s para evitar TimeoutError prematuro
        actual_timeout = max(remaining_time + 10, 30)
        
        self.logger.info(f"🔄 Lanzando reintentos en paralelo para {len(failed_ports)} puertos...")
        self.logger.info(f"   Tiempo restante: {remaining_time:.1f}s, Timeout: {actual_timeout:.1f}s")
        ...
        try:
            with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
                futures = [executor.submit(self._retry_single_port, *task) for task in retry_tasks]
            ...
            self.logger.warning(f"⚠️  Timeout en reintentos: {unfinished} threads no terminaron en {actual_timeout:.1f}s")
            self.logger.info(f"   Los threads restantes terminarán naturalmente al alcanzar round_end_time")
        except Exception as e:
            self.logger.error(f"❌ Error en _retry_failed_ports: {e}")
    
    def _retry_single_port(self, port: str, state: PortState, remaining_time: float, results: dict, round_end_time: datetime):
        """
        ...
        while self.running:
            ...
            if datetime.now() >= round_end_time:
                self.logger.warning(f"[{port}] ⏰ Límite de ronda alcanzado - Terminando retry thread")
                return
            ...
            if last_change_time is None:
                ...
                time_since_change = time.time() - last_change_time
                if time_since_change >= sim_attempt_time:
                    self.logger.warning(f"[{port}] ⏰ {time_since_change/60:.1f}min sin registro, cambiando SIM...")
                    needs_change = True
            
            if needs_change:
                # VERIFICACIÓN ADICIONAL antes de cambiar SIM
                if datetime.now() >= round_end_time:
                    self.logger.warning(f"[{port}] ⏰ Límite de ronda alcanzado - Terminando retry antes de cambio")
                    return
                ...
                if not success:
                    self.logger.warning(f"[{port}] ⚠️  No se pudo cambiar SIM, reintentando...")
                    time.sleep(5)
                    continue
                ...
                self.logger.info(f"[{port}] ⏳ Esperando estabilización ({reboot_stabilization}s)...")
                time.sleep(reboot_stabilization)
                
                self.logger.info(f"[{port}] 📡 Esperando registro de red ({network_reg_wait}s)...")
                ...
                try:
                    ...
                    if creg in ['1', '5']:
                        ...
                        self.logger.info(f"[{port}] ✅ Registrado ({network_type}) en {elapsed_minutes:.1f}min")
                        self.liberar_puerto(port)
                        results[port] = True
                        return  # Terminado con éxito
                    else:
                        self.logger.warning(f"[{port}] ⚠️  CREG={creg} después de cambio")
                except Exception as e:
                ...
                if time_since_change >= reboot_threshold_time and not reboot_done:
                    self.logger.info(f"[{port}] 🔄 Reboot adicional ({reboot_threshold_time/60:.1f}min sin registro)")
                    ...
                    try:
                        ...
                        if success:
                            ...
                            self.logger.warning(f"[{port}] ⚠️  Fallo en reboot adicional")
                    except Exception as e:
                        self.logger.error(f"[{port}] ❌ Error en reboot adicional: {e}")
                    
                    # VERIFICACIÓN ADICIONAL después de reboot
                    if datetime.now() >= round_end_time:
                        self.logger.warning(f"[{port}] ⏰ Límite de ronda alcanzado - Terminando retry")
                        return
                    ...
                    try:
                        ...
                        if creg in ['1', '5']:
                            network_type = "Home" if creg == '1' else "Roaming"
                            self.logger.info(f"[{port}] ✅ Registrado ({network_type}) tras reboot adicional")
                            self.liberar_puerto(port)
                            results[port] = True
                            return  # Terminado con éxito
                        else:
                            self.logger.info(f"[{port}] 🔍 CREG={creg} después de reboot adicional")
                    except Exception as e:
                        self.logger.error(f"[{port}] ❌ Error verificando CREG: {e}")
                else:
                    ...
                    try:
                        ...
                        if creg in ['1', '5']:
                            network_type = "Home" if creg == '1' else "Roaming"
                            self.logger.info(f"[{port}] ✅ Registrado ({network_type}) tras {time_since_change/60:.1f}min")
                            self.liberar_puerto(port)
                            results[port] = True
                            return  # Terminado con éxito
                        else:
                            self.logger.debug(f"[{port}] 📶 CREG={creg}, esperando... ({time_since_change/60:.1f}min)")
                    except Exception as e:
                        self.logger.error(f"[{port}] ❌ Error verificando CREG: {e}")
    ...
    def liberar_puerto(self, port: str):
        ...
        try:
            ...
            if state.last_monitor_time:
                elapsed_since_monitor = (datetime.now() - state.last_monitor_time).total_seconds()
                if elapsed_since_monitor < 5.0:
                    wait_time = 5.0 - elapsed_since_monitor
                    self.logger.info(f"⏳ {port}: Esperando {wait_time:.1f}s antes de liberar (5s desde último monitoreo)")
                    time.sleep(wait_time)
            
            state.mark_released()
            sim_numero = state.current_sim.get('numero', 'N/A') if state.current_sim else 'N/A'
            sim_ccid = state.current_sim.get('ccid', 'N/A') if state.current_sim else 'N/A'
            self.logger.info(f"🔓 {port}: Liberado a SimClient - Phone: {sim_numero}, CCID: {sim_ccid}")
        except Exception as e:
    ...
    def _cambiar_sim_sincronico(self, port: str, state: PortState):
        ...
        try:
            ...
            old_numero = state.current_sim.get('numero', 'N/A') if state.current_sim else 'N/A'
            old_ccid = state.current_sim.get('ccid', 'N/A') if state.current_sim else 'N/A'
            old_fila = state.current_sim.get('fila', 'N/A') if state.current_sim else 'N/A'
            ...
            self.logger.info(f"[{port}]    Anterior: Fila {old_fila}, Phone {old_numero}, CCID {old_ccid}")
            self.logger.info(f"[{port}]    Nueva:    Fila {fila}, Col {col}, Phone {numero}, CCID {ccid}")
            ...
            new_numero = state.current_sim.get('numero', 'N/A') if state.current_sim else 'N/A'
            new_ccid = state.current_sim.get('ccid', 'N/A') if state.current_sim else 'N/A'
            self.logger.info(f"[{port}] ✅ Cambio completado - Phone: {new_numero}, CCID: {new_ccid}")
    ...
    def _cambiar_al_siguiente_simcard_thread(self, port: str):
        ...
        try:
            ...
            # LOG DETALLADO: Transición completa
            self.logger.info(f"🔄 {port}: CAMBIO DE SIM")
            self.logger.info(f"   Anterior: Fila {old_fila}, Phone {old_numero}, CCID {old_ccid}")
            self.logger.info(f"   Nueva:    Fila {fila}, Col {col}, Phone {numero}, CCID {ccid}")
    ...
    def run_production_cycle(self):
        ...
        if not launch_ok:
            self.logger.warning("⚠️  SimClient no se lanzó, continuando de todas formas...")
        else:
        ...
        self.logger.info("PASO 4: Proceso paralelo de puertos (con monitoreo integrado)...")
    ...
    def run_continuous(self):
        ...
        try:
            while self.running:
                ...
                if not success:
                    self.logger.error("Ronda falló, esperando 5 minutos antes de reintentar...")
                    time.sleep(300)
...
def main():
    ...
    parser.add_argument('--resume', action='store_true',
                       help='Reanudar desde estado guardado')
    parser.add_argument('--cycle', action='store_true',
                       help='Ejecutar un solo ciclo (testing)')
            ...
            while True:
                ...
                if choice == '1':
                    ...
                    if orchestrator.slot_manager is None:
                        orchestrator.initialize_data(force_scan=False, generate_outputs=True)
                    ...
                    for port, info in status['ports'].items():
                        print(f"  {port}: {info['valid_sims']} SIMs, índice actual: {info['current_index']}")
                    print("=" * 70)
